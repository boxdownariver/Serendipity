 ...
 * This program generates a main menu interface for the Serendipity project
 *_________________________________________________________________________
 *	INPUTS-
 *	    Key input (char) -> Keyboard-based instruction for the program
 *	    Signals (int) -> Signals passed by the system
 *	    mainMenuInfo (char *, char **, char *) -> Name, menu options, store
 *	OUTPUTS-
...
int main() {
	...
	MenuLines mainMenuInfo = //INPUT- Information struct for main menu
			{ "Main Menu", { "Cashier Module", "Inventory Database Module",
					"Report Module", "Exit" } };
	...
	mvwprintw(mainWindow, getmaxy(mainWindow) - 2,
			...
			"Select [1-4] or navigate to module...");
	wrefresh(mainWindow);
